...
function buildSystemPrompt(state) {
  ...
  const parts = [
    ...
    '- Quote the offending clause text briefly (<=200 chars) in clause_quote. Use the EXACT verbatim wording from the agreement so downstream tools can highlight it inline.',
    '- The impact field MUST be concrete and human-emotional, not abstract legal language. Examples: "You may risk losing your Rs. 1.5 lakh deposit", "Your landlord could legally evict you with 24 hours notice under this clause", "You would have no legal recourse if rent is hiked mid-year". Quantify in rupees whenever the clause involves money.',
    '- Optionally include a market_comparison field per finding (one short sentence) when the clause is unusual vs. the Indian rental market. Examples: "Most Mumbai residential rentals cap deposit at 2-3 months", "Standard notice period across India is 1 month, not 3", "Industry norm is 18-24% per annum on late rent, not 5% per day". Omit (set to null) when the clause is not market-anomalous.',
    '- Add a SHORT simple_explanation per finding (1-2 sentences, plain English, no legalese). Example: "This means the landlord can keep your full deposit even if you move out properly." Aimed at non-lawyers.',
    '- Add a risk_contribution number per finding (0-30). High-risk findings should contribute 15-30, medium 8-15, low 3-8. The sum of contributions should roughly match how much they pull down the overall score.',
    '- Add a top-level landlord_questions array (3-5 short, polite, non-aggressive questions the tenant could ask their landlord to negotiate the problematic clauses). Examples: "Could we revise the deposit to 2 months in line with the MTA?", "Could the notice period be the standard one month?".',
    ...
    '    "market_comparison": "string-or-null",',
